...
A later generation of `rmarkdown` and `knitr`, `quarto`, was developed at Posit PBC; together these tools are part of the gold-standard for reproducible research across the sciences, in addition to making reporting easier within e.g. business and administrative units. Some of the charts and graphs put out by the Chancellor's office were assembled using `ggplot2` and either `rmarkdown` or `quarto`.
