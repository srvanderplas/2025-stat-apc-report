...
read_csv("data-analysis/data/academic-analytics/ri_department_faculty_2025-9-27_9-22-45.csv") |>
  group_by(unitname) |>
  count() |>
  arrange(n) |> 
  mutate(unitname = str_remove(unitname, ", Department of$") |>
           str_remove(",? School of$"))  |>
  rename(Unit = unitname, Size = n) |>
  mutate(Status = if_else(Unit %in% eliminate, "Eliminate", if_else(Unit %in% merger1, "Combine (1)", if_else(Unit %in% merger2, "Combine (2)", "")))) |>
  knitr::kable(caption = "Department Size and Budget Proposal Status")
```
