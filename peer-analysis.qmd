...
Rankings among R1 statistics departments have been [removed at the request of university counsel](2025 1014 Academic Analytics.pdf).
...
Rankings among land grant statistics departments have been [removed at the request of university counsel](2025 1014 Academic Analytics.pdf).

<!-- UNL Statistics ranks `r rank_lg`th of `r n_lg` land grant university statistics departments, or in the `r sprintf("%0.2f", percentile_lg)`th percentile.  -->
...
<!-- UNL Statistics ranks `r rank_b10`th of `r n_b10` Big Ten university statistics departments, or in the `r sprintf("%0.2f", percentile_b10)`th percentile.  -->
Rankings among Big Ten statistics departments have been [removed at the request of university counsel](2025 1014 Academic Analytics.pdf).


```{r}
#| fig-width: 8
#| fig-height: 5
#| fig-cap: "Comparison of UNL to other Big Ten statistics departments. UNL's SRI is just slightly below the median of all Big Ten statistics unit SRIs. USC and Oregon are not shown because Academic Analytics does not have statistics research ratings for those universities. Big Ten universities are shown in blue, with UNL in red. The original figure showed school colors and but was modified [at the request of general counsel](2025 1014 Academic Analytics.pdf)."
library(ggrepel)
# Read the CSV file
...
stats_full_ten <- stats_full |>
  ...
  mutate(primary_color = ifelse(abbreviation=="UNL", "#d00000", "#0088CE"),
         secondary_color = ifelse(abbreviation=="UNL", "#d00000", "#0088CE"))
...
stats_full_ten |>
  ...
  geom_dotplot(aes(fill = primary_color, color = primary_color),
  ...
  theme_bw() + 
  xlab("SRI") + ylab("") + 
  ggtitle("SRI of Statistics Departments by Carnegie Classification and AAU membership")
...
<!-- UNL Statistics ranks `r rank_aau`rd of `r n_aau` AAU statistics departments, or in the `r sprintf("%0.2f", percentile_aau)`th percentile.  -->
Rankings among AAU statistics departments have been [removed at the request of university counsel](2025 1014 Academic Analytics.pdf).
...
<!-- UNL Statistics ranks `r rank_public_aau`rd of `r n_public_aau` public university statistics departments in the AAU, or in the `r sprintf("%0.2f", percentile_public_aau)`th percentile.  -->
Rankings among public AAU statistics departments have been [removed at the request of university counsel](2025 1014 Academic Analytics.pdf).
...
We fear that if the Statistics department is cut, UNL will soon lose not only any hope of rejoining the AAU, but may also find its place in the Big Ten threatened. 
If it continues on the same course, even the R1 status of the university could be in jeopardy.
...
This damage will take multiple generations to repair^[Or a *very* significant investment of funding both from philanthropy and from the university itself to demonstrate a lasting commitment to the discipline.], even if the administration sees the error of eliminating our department relatively quickly. 
