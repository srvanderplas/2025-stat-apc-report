...
#| fig-width: 10
#| fig-height: 7.5
#| fig-cap: "Research performance (SRI percentile) of units by classification. UNL is performing extremely well compared to its peers and has better performance than many AAU institutions. Land grant institutions are shown in green, with UNL in red (also labeled). This chart has been modified at the request of [UNL's Provost and general counsel](2025 1014 Academic Analytics.pdf) to remove labeled departments that are well-known to have good statistics programs; they have been replaced by a generic comparison to other land grant institutions."
...
stats_full_dot <- stats_full |>
  select(institutionname, facultycount, sri, Carnegie, `AAU Member`, `Land Grant`) |>
  mutate(
    Carnegie_short = 
  ...
  mutate(
    color = if_else(`Land Grant` == "Land Grant", "#006747", "grey"),
    color = if_else(institutionname != "University of Nebraska - Lincoln", color, "#d00000"),
    shortname = if_else(institutionname == "University of Nebraska - Lincoln", "UNL", "NA")
  ) |>
...
unl <- stats_full_dot |> filter(institutionname=="University of Nebraska - Lincoln")
...
stats_full_dot |> 
  ...
  geom_text(aes(label = shortname, color = color), nudge_y =-.25, data = unl) 
...
#| fig-width: 10
#| fig-height: 7.5
#| fig-cap: "Size of Statistics-related Units at each institution. UNL is managing to handle all of its BS, MS, and PhD programs within a department that is extremely small relative to peer institutions at both R1 and AAU institutions.  This chart has been modified at the request of [UNL's Provost and general counsel](2025 1014 Academic Analytics.pdf) to remove labeled departments that are outliers and/or universities with good statistics programs; they have been replaced by a generic comparison to other land grant institutions."
...
stats_full_dot <- stats_full |>
  select(institutionname, unitname, facultycount, sri, Carnegie, `AAU Member`, `Land Grant`) |>
  mutate(
    Carnegie_short = 
      ifelse(Carnegie == "Doctoral Universities: Very High Research Activity", "R1",
             ifelse(Carnegie == "Doctoral Universities: High Research Activity", "R2", "Other")),
...
stats_full_dot |>
  ...
  geom_text(
    ...
    data = filter(stats_full_dot, shortname=="UNL", color != "grey"), nudge_x = 0, nudge_y = .2)
