...
unl_depts_sum |>
  ...
  scale_x_continuous("SRI Percentile (vs. all departments in AcA)", limits = c(0, 100), expand=expand_scale(mult = c(.1, 0), add = c(.1, 0))) +
...
stats_full_dot <- stats_full |>
  ...
  left_join(colors) |>
  mutate(color = if_else(is.na(color) | shortname != "UNL", "grey", color)) |>
...
stats_full_dot |> 
  ...
  geom_text(aes(label = shortname, color = color), data = filter(stats_full_dot, shortname=="UNL", color != "grey" & !is.na(nudge_x)),
